...
Project.prototype = {
	...
	//opens location in file browser, such as Finder or Windows Explorer
	//
	...
	openInFileBrowser: function( fn ) {
		...
		console.log("launch with command: "+ cmd );

		spawn( cmd, [this.location] ).on('exit', function( code ) {
			if( code !== 0 ){
				console.log( colors.red + 'Error opening folder in file browser, exited with code '+ code );
			}
			if( fn ) fn( code );
		});
		return this;
