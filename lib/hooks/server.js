...
exports.tryToMakeServer = function( app, opts, callback ){
    ...
    opts.incrementPortOnError = !(opts.incrementPortOnError === false);
    ...
    function tryServer(){
        ...
        server = http.createServer(app)
            .listen( opts.port, function(){
                callback( null, server, app, opts.port );
            })
            .on('error', function(error){
                if( opts.incrementPortOnError ){
                    opts.port++;
                }
                if( numAttempts < opts.maxAttempts ){
                    tryServer();
                } else {
                    callback(error, server, app, opts.port);
                }
            });
    }
