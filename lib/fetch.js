...
function fromGit( resource, options, callback ){
	...
	console.log('prepping fetch from git: ' + tmpStorage + ' ,  '+ repoSourceFolder);
	...
	async.waterfall([
		...
		function cloneRepo( next ){
			console.log('PREPARING TO CLONE');
        ...
        function rmGitFolder( next ){
					console.log('rm git folder');
					var gitRefFolder = path.join(repoSourceFolder, '.git');
           wrench.rmdirRecursive( gitRefFolder, function(err){
						console.log('fin .git');
						if( err ){
