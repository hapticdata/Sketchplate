	...
	async.waterfall([
		function process( next ){
			if( typeof resource.target === 'string' ){
				//copy entire thing
                next( null, [{ source: '', destination: resource.target }]);
			} else {
                var keys = _.keys(resource.target);
                async.map( keys, function(k, fn){ glob(k,{cwd: contentsFolder}, fn); }, function( err, globs ){
                    if( err ){
                        next(err);
                        return;
                    }
                    //structure: [ [destination, [source, source] ]
                    var targets = _.zip(_.values(resource.target), globs);
                    var targetIndex = -1;
                    var mapProcess = function(arr, callback){
                        targetIndex++;
                        var dest = arr[0];
                        var sources = arr[1].map(function(src){ return contentsFolder + src; });
                        //for every source in the nested array, map to its destination
                        //globPattern is needed to know our resulting directory structure
                        (function(globPattern){
                            async.mapSeries(sources, fs.stat, function( err, stats ){
                                if( err ){
                                    callback( err );
                                    return;
                                }
                                var maps = stats.map(function(stat, i){
                                    //arr[1][i] is the relative path
                                    var _src = arr[1][i];
                                    if( globPattern.indexOf('**/*') > 0 ){
                                        var start = globPattern.slice(0,globPattern.indexOf('**/*'));
                                        _src = _src.slice(start.length);
                                        //if it was a directory and it was from a glob
                                        //dont do it, a file will do it later
                                        if( stat.isDirectory() ){
                                            return undefined;
                                        }
                                    }
                                    //if( stat.isFile() && dest.lastIndexOf(_src) === dest.length - _src.length ){
                                    //    dest = dest.slice(0,dest.length-_src.length+1);
                                    //}
                                    return {
                                        //sources[i] is the full path
                                        source: sources[i],
                                        destination: stat.isDirectory() ? dest : path.join(dest,_src)
                                    };
                                });
                                //get rid of any that were undefined (intentionally)
                                callback( null, _.compact(maps) );
                            });
                        })(keys[targetIndex]);
                    };

                    async.mapSeries( targets, mapProcess, function(err, allMaps){
                        next( err, err ? null : _.flatten(allMaps) );
                    });
                });
			}
		},
		function copy( targets, next ){
            async.forEach(targets, function( target, callback){
                ...
                var write = function write(){
                    fs.stat(target.source, function(err, stat){
                        if( stat.isFile() ){
                            var writeStream = fs.createWriteStream(target.destination);
                            ...
                            fs.createReadStream(target.source).pipe(writeStream);
                        } else if( stat.isDirectory() ){
                            wrench.copyDirRecursive( target.source, target.destination, callback );
                        }
                    });
                };
