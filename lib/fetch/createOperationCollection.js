...
module.exports = exports = function( tempDirectory, resource, callback ){
    ...
    //ensure tempDirectory ends with OS-appropriate separator
    tempDirectory = path.normalize(tempDirectory);
    if('/' !== path.sep && tempDirectory.lastIndexOf('/') === tempDirectory.length -1){
        tempDirectory = tempDirectory.slice(0, -1);
    }
    ...
    var resolveMap = function( sourceMap, callback ){
        ...
        var maps = _.map( sourceMap, function( relSrcs, glob ){
            ...
            return _.map( relSrcs, function(src){
                var absSrc = path.join(tempDirectory, src);
                var g = glob;
                ...
                var bfIndex = dest.lastIndexOf(badFilename);
                if( bfIndex !== -1 && bfIndex === (dest.length - badFilename.length) ){
